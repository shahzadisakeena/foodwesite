...
export default function CraftSection() {
  const items = [
    { title: "Handcrafted Dough", desc: "Prepared fresh daily." },
    { title: "Stone Oven Baking", desc: "Perfect crispy crusts." },
    { title: "Premium Ingredients", desc: "Fresh & high-quality." },
  ];

  return (
    <section className="py-18 ">
      <div className="container w-[90vw] mx-auto grid md:grid-cols-3 gap-10">
        {items.map((item, i) => (
          <div
            key={i}
            className="bg-white/70 backdrop-blur rounded-2xl p-8 text-center shadow-lg"
          >
            <h3 className="text-xl font-bold text-stone-red mb-3">
              {item.title}
            </h3>
            <p className="text-stone-deep/80">{item.desc}</p>
          </div>
        ))}
      </div>
    </section>

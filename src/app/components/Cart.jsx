...
const Cart = () => {
  ...
  return (
    <section className="min-h-screen py-24">
      <div className="w-[92vw] lg:w-[85vw] mx-auto grid lg:grid-cols-[2fr_1fr] gap-10">
        {/* ================= LEFT CART ================= */}
        <div className="stone-card relative">
          {/* BACK BUTTON */}
          <Link href="/">
            <div className="absolute top-6 left-6 bg-stone-red text-white p-2 rounded-full shadow-lg hover:scale-110 transition">
              <ChevronLeft />
            </div>
          </Link>

          <h2 className="text-2xl font-bold text-stone-deep border-b pb-4 text-center">
            Items From Your Cart
          </h2>
          ...
          <div className="flex flex-col items-center justify-center py-12 text-center">
            ...
            <h3 className="text-lg lg:text-xl font-semibold text-stone-deep mt-6">
              You haven’t added any items yet
            </h3>

            <Link href="/menu">
              <button className="mt-6 bg-stone-red hover:bg-stone-orange text-white px-8 py-3 rounded-xl font-semibold shadow-lg transition">
                Explore Menu
              </button>
            </Link>
          </div>
        ...
        {/* ================= ORDER SUMMARY ================= */}
        <div className="stone-card h-fit">
          <h1 className="text-xl font-bold text-stone-deep border-b pb-3">
            0 Items Added
          </h1>

          {/* SPECIAL INSTRUCTIONS */}
          <div className="stone-subcard mt-4">
            <h3 className="text-stone-deep font-semibold mb-2">
              Special Instructions (Optional)
            </h3>

            <textarea
              placeholder="Additional instructions..."
              className="stone-textarea"
            />

            {/* PHONE INPUT */}
            <FormControl variant="filled" fullWidth className="mt-4">
              <InputLabel className="!text-stone-deep/70">
                Alternate Phone Number (3XXXXXXXXX)
              </InputLabel>
              ...
              <FilledInput
                ...
                onChange={(e) => {
                  ...
                  if (!isNaN(val) && val.startsWith("3") && val.length <= 10) {
                    setNumber(val);
                  }
                }}
                startAdornment={
                  <InputAdornment position="start">
                    <Typography className="text-stone-deep border-r pr-2 font-semibold">
                      +92
                    </Typography>
                  </InputAdornment>
                }
                className="!bg-white/80 rounded-xl mt-3"
              />
            </FormControl>
          </div>

          {/* ORDER TOTAL */}
          <div className="stone-subcard mt-5">
            <h2 className="text-2xl font-bold text-center text-stone-deep mb-4">
              Your Order
            </h2>

            <PriceRow label="Sub Total" value={subtotal} />
            <PriceRow label="Delivery Fee" value={deliveryCharge} />
            <PriceRow label="GST (17%)" value={gst} />

            <div className="flex justify-between font-bold text-lg border-t pt-4 text-stone-deep">
              <span>Total</span>
              <span>Rs. {total.toFixed(2)}</span>
            </div>
          </div>
          ...
          {/* CHECKOUT */}
          <button className="w-full mt-6 bg-stone-red hover:bg-stone-orange text-white py-3 rounded-xl font-semibold text-lg shadow-lg transition">
            Checkout
          </button>
        </div>
      </div>
...
const PriceRow = ({ label, value }) => (
  <div className="flex justify-between text-stone-deep/80 my-3">
    <span>{label}:</span>

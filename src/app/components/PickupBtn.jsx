  ...
  return (
    <div>
      <Dialog open={isOpen} onOpenChange={setIsOpen}>
        ...
        {/* ✅ Dialog */}
        <DialogContent className="rounded-xl bg-stone-bg text-stone-deep h-[90vh] max-w-xl">
          <DialogTitle />
          ...
          {/* Body */}
          <div className="mt-4 mb-4 overflow-y-auto pr-2">
            {addresses.map((add, index) => (
              <div
                key={index}
                tabIndex={0}
                ...
                className="pb-4 px-3 border-b border-stone-border cursor-pointer rounded-md transition-all
                hover:bg-white focus:bg-stone-yellow/30"
              >
                <h5 className="text-lg font-semibold">{add.location}</h5>

                <p className="text-sm text-gray-600">{add.address}</p>

                <h5 className="text-md font-semibold text-stone-red mt-1">
                  {add.startTime} - {add.endTime}
                </h5>

                <div className="flex justify-between items-center mt-2">
                  <span className="text-sm">Distance: {add.distance}</span>

                  <button
                    type="button"
                    className="flex items-center px-3 py-1 bg-stone-red text-white text-sm rounded hover:bg-stone-orange transition"
                  >
                    Get Direction
                    <CornerUpRight size={14} className="ml-2" />
                  </button>
                </div>
              </div>
            ))}
          ...
          <div className="border-t border-stone-border pt-3">
            <button
              ...
              className={`w-full h-11 rounded-md font-semibold transition ${
                locationSelect.isTrue
                  ? "bg-stone-red text-white hover:bg-stone-orange"

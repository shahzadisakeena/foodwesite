  ...
  return (
    <main className="bg-stone-bg py-14 lg:py-24">
      ...
      <div className="w-[92vw] max-w-5xl mx-auto">
        {/* CARD WRAPPER */}
        <div className="bg-white/70 backdrop-blur-xl rounded-3xl shadow-xl p-6 md:p-10 lg:p-14 border border-stone-border">
          {/* HEADER */}
          <div className="text-center mb-12">
            <h1 className="text-3xl md:text-4xl font-extrabold text-stone-deep">
              Privacy Policy
            </h1>
...
const Section = ({ title, text, list }) => {
  return (
    <div>
      <h3 className="text-lg md:text-xl font-bold text-stone-deep mb-3">
        {title}
      </h3>

      {text && (
        <p className="text-stone-deep/80 text-sm md:text-base">{text}</p>
      )}

      {list && (
        <ul className="list-disc ml-5 space-y-2 text-stone-deep/80 text-sm md:text-base">
          {list.map((item, i) => (
            <li key={i}>{item}</li>
          ))}
        </ul>
